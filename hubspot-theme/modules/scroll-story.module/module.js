...
(function () {
  ...
  document.querySelectorAll('[data-sstory]').forEach(function (root) {
    ...
    function paint() {
      ...
      set(title, '--clip', (100 - kf(p, [[0, 0], [10, 100]])).toFixed(2) + '%');
      set(title, '--o', kf(p, [[13, 1], [19, 0]]).toFixed(3));
      set(title, '--s', kf(p, [[13, 1], [19, .9]]).toFixed(3));
      set(title, '--ty', kf(p, [[13, 0], [19, -18]]).toFixed(1) + 'px');
      set(lede, '--o', kf(p, [[8, 0], [12, 1], [15, 1], [19, 0]]).toFixed(3));
      set(plate, '--o', kf(p, [[6, 0], [16, 1], [84, 1], [94, .18]]).toFixed(3));
      set(plate, '--s', kf(p, [[6, .84], [16, 1], [84, 1], [94, 1.08]]).toFixed(3));
      set(plate, '--tz', kf(p, [[6, -120], [16, 0], [84, 0], [94, 60]]).toFixed(1) + 'px');
      edges.forEach(function (e) { set(e, '--edge', kf(p, [[10, 0], [20, 1]]).toFixed(3)); });
      ...
      set(track, '--o', kf(p, [[10, 0], [16, 1], [86, 1], [94, 0]]).toFixed(3));
